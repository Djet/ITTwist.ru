...
```sh
ipsec-gw#sh ip arp | i 172.1.1.74
ipsec-gw#clear arp-cache 172.1.1.6
```
...
ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAgQCG0MO4Cv1HrCwaUAoU4ofO7DLJp4a1mqsg
+QKI+FGm043XR0pCX73faHI/D2BS+soBOhaOwTXLzYRJ8kH36pEIvdCW+E077wov
...
```sh
tshark -iany -f 'port 80 and host 172.1.1.8' -l -t ad -n -R 'http.request' -T fields -e http.host -e http.request.uri
```
 
Посмотреть статистику по использованию CPU за день

Она собирается в debian по умолчанию каждые 10 минут командой atsa и складывается в бинарный лог. С сортировкой по первому load average.
...
```sh
ipmitool -I lanplus -H 91.100.6.178 -U ipmi -P w**** sensor | grep "Ambient Temp" | tr -s ' ' | cut -d' ' -f 4
```
...
```sh
find /opt/router/server/default/log/ADR-????-??-$(date --date="1 day ago" +%d)-??.log -exec mv {} /backup/router/$(date +%Y)/$(date +%m)
find /backup/router/$(date +%Y)/$(date +%m)/ADR-????-??-$(date --date="1 day ago" +%d)-??.log -exec tar czf '{}.tgz' '{}'
find /backup/router/$(date +%Y)/$(date +%m)/ADR-????-??-$(date --date="1 day ago" +%d)-??.log -exec rm {}
```
 
Пример сформированной строки поиска:

```sh
/backup/router/2013/06/ADR-2013-06-13-00.log
```
...
```sh
(echo "sh run"; sleep 3; exit;) | ssh admin@172.1.1.111
```
...
CREATE ROLE tcp_test ENCRYPTED PASSWORD 'md5*****************************' NOSUPERUSER CREATEDB CREATEROLE INHERIT LOGIN;
```
...
```sh
CREATE USER nagios WITH PASSWORD 'passwd';
...
```sh
mutt -s DUMP -a 217.8.39.34_900.pcap -- user@server.com
```
...
Пользователь на чтение по умолчанию: asgro
...
Если нужны дополнительная права (исключения по обновлению или записи), смотрим здесьhttp://www.postgresql.org/docs/9.1/interactive/sql-grant.html
...
GRANT ALL PRIVILEGES ON databasename*.*\* TO "user"@"%" IDENTIFIED BY '*\*******' WITH GRANT OPTION;
```
 
Смена пароля у пользователя Mysql

```sh
UPDATE user SET password=PASSWORD('passwd') where User='debian-sys-maint';
flush privileges;
```
 
Добавление RO доступа к MySQL для пользователей toe

```sh
GRANT SELECT ON \*.\* TO "asgro"@"%" IDENTIFIED BY 'Chooghi8';
```
...
Удалиить все маршруты через 172.1.1.254 ипишник используя xargs

```sh
ip route | grep 172.1.1.254 | awk '{print $1}' | xargs -i ip route del {}
```
...
```sh
tshark -ibond0 -f "host 92.100.100.2 and port 3333" -d tcp.port==3333,smpp
```
 
Проброс порта из офисной сети до сайта через сеть дц.
...
```sh
root@ss4:~# wget https://github.com/flexiondotorg/oab-java6/archive/master.zip
root@ss4:~# unzip master.zip
root@ss4:~# cd oab-java6-master/
root@ss4:~/oab-java6-master# ./oab-java.sh -7
root@ss4:/# apt-get update
root@ss4:/# apt-get install oracle-java7-jre
root@ss4:/# sudo update-alternatives --config java
...
```sh
mysql> show create database router;
+-------------+----------------------------------------------------------------------+
| Database    | Create Database                                                      |
+-------------+----------------------------------------------------------------------+
| router | CREATE DATABASE `router` /*!40100 DEFAULT CHARACTER SET utf8 */ |
+-------------+----------------------------------------------------------------------+
1 row in set (0.00 sec)
 
root@java2:~# mysqldump -d router > ~/router_dump_19_03_2013_schema_only.sql
root@java2:~# scp ~/router_dump_19_03_2013_schema_only.sql trn:~/
```
...
mysql> GRANT ALL PRIVILEGES ON router.* TO "router"@"%" WITH GRANT OPTION;
mysql> flush privileges;
 
root@ss4:~# mysql -u router -p router < ~/router_dump_19_03_2013_schema_only.sql
```
...
```sh
GRANT ALL PRIVILEGES ON *.* TO 'pt_chtables'@'172.1.1.54' IDENTIFIED BY PASSWORD 'passwd';
GRANT ALL PRIVILEGES ON `lite_icb_demo`.* TO 'pt_chtables'@'172.1.1.54';
GRANT ALL PRIVILEGES ON `lite_icb`.* TO 'pt_chtables'@'172.1.1.54';
```
...
```sh
webhost2:/var/log/nginx# ls  | sort -t '.' -nk 4 | grep archipelag.error
```
...
```sh
root@ss2:# cat /boot/config-2.6.32-5-amd64|grep NETCONSOLE
CONFIG_NETCONSOLE=m
...
```sh
root@ssc2:# modprobe netconsole netconsole=6969@10.90.30.130/eth0,6969@10.90.30.129/5c:f3:fc:b6:05:b0
root@ssc2:# dmesg | tail
...
3)
Запускаем nc на сервере где будем ловить логи

```sh 
root@ssc1:# nc -u -l -p 6969
```
